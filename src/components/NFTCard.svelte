<div
  class="place-self-center bg-cstm-neutral-dark-blue-card-bg m-6 p-6 rounded-lg max-w-sm"
>
  <div class="text-cstm-primary-soft-blue font-outfit">
    <div class="relative rounded-lg">
      <div class="peer cursor-pointer">
        <img
          src="assets/image-equilibrium.jpg"
          alt="equilibrium"
          class="rounded-lg"
        />
      </div>
      <div
        class="absolute w-full h-full inset-0 rounded-lg [background:rgba(0,255,247,0.5)] hidden peer-hover:block"
      >
        <img
          src="assets/icon-view.svg"
          alt="eye icon"
          class="relative top-[43%] left-[43%]"
        />
      </div>
    </div>
    <h1
      class="text-white hover:text-cstm-primary-cyan hover:cursor-pointer font-semibold text-2xl my-5"
    >
      Equilibrium #3429
    </h1>
    <p class="text-xl">Our quilibrium collection promotes balance and calm.</p>
    <div class="flex justify-between items-center">
      <div class="flex gap-2 my-5">
        <img
          src="assets/icon-ethereum.svg"
          alt="ethereum icon"
          class="self-center"
        />
        <p class="text-cstm-primary-cyan">0.041 ETH</p>
      </div>
      <div class="flex gap-2">
        <img src="assets/icon-clock.svg" alt="clock icon" class="self-center" />
        <p>3 days left</p>
      </div>
    </div>
    <div class="w-full h-[1px] bg-cstm-neutral-dark-blue-line-bg mb-5" />
    <div class="flex items-center gap-4">
      <img
        src="assets/image-avatar.png"
        alt="person avatar"
        width="40px"
        height="40px"
        class="border-2 border-white rounded-full"
      />
      <p>
        Creation of <span
          class="text-white hover:text-cstm-primary-cyan hover:cursor-pointer"
          >Jules Wyvern</span
        >
      </p>
    </div>
  </div>
</div>
